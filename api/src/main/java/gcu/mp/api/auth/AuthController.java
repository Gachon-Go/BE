...
public class AuthController {
    ...
    public ResponseEntity<BaseResponse<String>> signUp(@RequestBody CreateMemberRequest createMemberRequest) {
        try {
            ...
            if (memberService.existMember(authMapper.toOauthMemberDto(oAuth2UserInfo))) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new BaseResponse<>(BaseResponseStatus.EXISTS_MEMBER));
            }
            if (memberService.existNickname(createMemberRequest.getNickname())) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new BaseResponse<>(BaseResponseStatus.EXISTS_MEMBER_NICKNAME));
            }
            if (memberService.existEmail(createMemberRequest.getEmail())) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new BaseResponse<>(BaseResponseStatus.EXISTS_EMAIL));
            }
    ...
    public ResponseEntity<BaseResponse<LogInMemberResponse>> logIn(@RequestBody LoginMemberRequest loginMemberRequest) {
        try {
            ...
            if (memberId == null)
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new BaseResponse<>(BaseResponseStatus.NOT_EXIST_MEMBER));
